...
use ulpf_ai::onboarder::{Onboarder, DynamicParserRegistry};
...
async fn run_ingest(args: IngestArgs) -> Result<()> {
    ...
    println!("\x1b[1;32m   ULPF - Universal Log Pre-processing & Cryptographic Integrity Fabric\x1b[0m");
    println!("\x1b[1;36m====================================================================\x1b[0m");
    ...
    println!("  Batch Trigger     : {} logs OR {} ms", args.batch_size, args.batch_timeout);
    ...
    tokio::spawn(async move {
        ...
        loop {
            ...
            let eps = if elapsed > 0.0 { diff as f64 / elapsed } else { 0.0 };
    ...
    println!("\x1b[1;32m[+] Engine active. Listening for Syslog UDP/TCP traffic on port 5140...\x1b[0m");
...
fn run_verify(args: VerifyArgs) -> Result<()> {
    println!("\x1b[1;36m====================================================================\x1b[0m");
    println!("\x1b[1;32m           ULPF Cryptographic Integrity & Forensics Auditor         \x1b[0m");
    println!("\x1b[1;36m====================================================================\x1b[0m");
    ...
    if !args.file.exists() {
        println!("\x1b[1;31m[ERROR] Parquet block file does not exist: {}\x1b[0m", args.file.display());
        std::process::exit(1);
    }

    if !args.ledger.exists() {
        println!("\x1b[1;31m[ERROR] Cryptographic ledger file does not exist: {}\x1b[0m", args.ledger.display());
        std::process::exit(1);
    }
    ...
    println!("  Ledger Merkle Root     : \x1b[1;34m{}\x1b[0m", report.ledger_merkle_root);
    println!("  Computed Merkle Root   : \x1b[1;34m{}\x1b[0m", report.computed_merkle_root);
    ...
    if report.is_valid {
        ...
        println!("\n\x1b[1;41;37m   [ALARM] FORENSIC TAMPERING DETECTED! INTEGRITY COMPROMISED!   \x1b[0m");
        println!("\x1b[1;31m✘ Corrupted Records Detected: {}\x1b[0m\n", report.tampered_records.len());

        for (idx, record) in report.tampered_records.iter().enumerate() {
            println!("  \x1b[1;33m[Tamper Event #{}]\x1b[0m Leaf Index: {}", idx + 1, record.leaf_index);
...
fn run_onboard(args: OnboardArgs) -> Result<()> {
    println!("\x1b[1;36m====================================================================\x1b[0m");
    println!("\x1b[1;32m              ULPF Air-Gapped AI Device Onboarder                   \x1b[0m");
    println!("\x1b[1;36m====================================================================\x1b[0m");
    ...
    if !args.sample.exists() {
        println!("\x1b[1;31m[ERROR] Sample log file not found: {}\x1b[0m", args.sample.display());
        std::process::exit(1);
    }
    ...
    println!("[*] Ingested {} sample raw events for pattern analysis...", sample_lines.len());
    ...
    println!("[+] Synthesis completed in \x1b[1;32m{:.2?}\x1b[0m", elapsed);
    println!("  Validation Pass Rate: \x1b[1;32m{:.1}%\x1b[0m ({} of {} samples passed)", report.match_percentage, report.matched_samples, report.total_samples);
    println!("  Synthesized Regex   : \x1b[1;33m{}\x1b[0m", parser_def.regex_pattern);

    fs::create_dir_all(&args.out)?;
    let json_path = args.out.join(format!("{}.json", args.vendor.to_lowercase()));
    let yaml_path = args.out.join(format!("{}.yaml", args.vendor.to_lowercase()));
    ...
    println!("  Source EP     : {}:{:?}", test_event.src_endpoint.ip.as_deref().unwrap_or("N/A"), test_event.src_endpoint.port);
    println!("  Dest EP       : {}:{:?}", test_event.dst_endpoint.ip.as_deref().unwrap_or("N/A"), test_event.dst_endpoint.port);
    println!("  Protocol      : {:?}", test_event.connection_info.protocol_name);
...
async fn run_benchmark(args: BenchmarkArgs) -> Result<()> {
    println!("\x1b[1;36m====================================================================\x1b[0m");
    println!("\x1b[1;32m               ULPF Multi-Core Throughput Benchmark                \x1b[0m");
    println!("\x1b[1;36m====================================================================\x1b[0m");
    ...
    if corpus.is_empty() {
        println!("\x1b[1;31m[ERROR] No logs found in {}. Run harvest first!\x1b[0m", args.data_dir.display());
        std::process::exit(1);
    }

    println!("[+] Loaded {} diverse raw perimeter log lines into RAM.", corpus.len());
    println!("[*] Starting {} worker tasks across CPU cores for {} seconds...", args.threads, args.duration);
    ...
    println!("  Elapsed Duration        : \x1b[1;37m{:.2}s\x1b[0m", elapsed);
    println!("  Aggregate Throughput    : \x1b[1;32m{:>10.0} Events / Second (EPS)\x1b[0m", eps);
    println!("  Per-Core Throughput     : \x1b[1;33m{:>10.0} EPS / thread\x1b[0m", eps / args.threads as f64);
...
// -----------------------------------------------------------------------------
// 5. PARQUET FORENSIC RECORD INSPECTOR
// -----------------------------------------------------------------------------

fn run_inspect(args: InspectArgs) -> Result<()> {
    println!("\x1b[1;36m====================================================================\x1b[0m");
    println!("\x1b[1;32m           ULPF Parquet Forensic Record Inspector                   \x1b[0m");
    println!("\x1b[1;36m====================================================================\x1b[0m");
    println!("  Parquet Block : {}", args.file.display());
    println!("\x1b[1;36m--------------------------------------------------------------------\x1b[0m");

    if !args.file.exists() {
        println!("\x1b[1;31m[ERROR] Parquet file does not exist: {}\x1b[0m", args.file.display());
        std::process::exit(1);
    }
    ...
    for (i, rec) in records.iter().take(args.count).enumerate() {
        ...
        let preview = if rec.raw_log.len() > 95 { format!("{}...", &rec.raw_log[..95]) } else { rec.raw_log.clone() };
        ...
        if let Ok(ocsf) = serde_json::from_str::<serde_json::Value>(&rec.ocsf_json) {
            ...
            println!("  • Activity ID       : {} ({})", ocsf.get("activity_id").unwrap_or(&serde_json::Value::Null), ocsf.get("activity_name").and_then(|v| v.as_str()).unwrap_or(""));
            println!("  • Disposition       : \x1b[1;32m{}\x1b[0m", ocsf.get("disposition").and_then(|v| v.as_str()).unwrap_or(""));
            if let Some(src) = ocsf.get("src_endpoint") {
                println!("  • Source Endpoint   : {}:{}", src.get("ip").and_then(|v| v.as_str()).unwrap_or("N/A"), src.get("port").unwrap_or(&serde_json::Value::Null));
            }
            if let Some(dst) = ocsf.get("dst_endpoint") {
                println!("  • Dest Endpoint     : {}:{}", dst.get("ip").and_then(|v| v.as_str()).unwrap_or("N/A"), dst.get("port").unwrap_or(&serde_json::Value::Null));
            }
            if let Some(conn) = ocsf.get("connection_info") {
                println!("  • Protocol          : {} (num: {})", conn.get("protocol_name").and_then(|v| v.as_str()).unwrap_or(""), conn.get("protocol_num").unwrap_or(&serde_json::Value::Null));
            }
        }
    }
...
fn run_tamper(args: TamperArgs) -> Result<()> {
    println!("\x1b[1;36m====================================================================\x1b[0m");
    println!("\x1b[1;33m       ULPF Adversary Forensic Tampering Injection Tool             \x1b[0m");
    println!("\x1b[1;36m====================================================================\x1b[0m");
    ...
    if !args.file.exists() {
        println!("\x1b[1;31m[ERROR] Target file does not exist: {}\x1b[0m", args.file.display());
        std::process::exit(1);
    }

    let mut records = ulpf_integrity::storage::read_parquet_file(&args.file)?;
    if args.leaf >= records.len() {
        println!("\x1b[1;31m[ERROR] Leaf index {} out of bounds (block has {} records)\x1b[0m", args.leaf, records.len());
        std::process::exit(1);
    }
    ...
    let tampered_log = if ip_regex.is_match(&original_log) {
        ip_regex.replace(&original_log, args.ip.as_str()).to_string()
    } else {
    ...
    println!("  • Altered Raw Log Snippet:\n    \x1b[1;31m{}\x1b[0m", &tampered_log[..tampered_log.len().min(90)]);
    println!("\n[*] Parquet archive rewritten with stealth modification.");
    println!("[*] Run '\x1b[1;36mulpf verify --file {}\x1b[0m' to audit.", args.file.display());
